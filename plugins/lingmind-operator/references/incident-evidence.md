...
3. Workload readiness, images, replicas, restarts, and rollout state.
4. Pod conditions and recent namespace events.
5. Narrow bounded container logs.
...
alone when concrete workload, pod, event, log, or rollout evidence is available. Metrics, persisted service
configuration, endpoint probes, and previous-container logs are not part of the current Operator catalog; report that
gap rather than inventing evidence.
