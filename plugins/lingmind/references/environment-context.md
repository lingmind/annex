...
One installed LingMind plugin may expose multiple direct Phoenix MCP connections. Every connection is named
`lingmind-<environment-code>` and remains bound to exactly one business environment, Keycloak issuer, OAuth audience,
and token set.

## Selection

1. If the user supplies an environment code, select only the connection with that exact normalized code.
2. If no environment is supplied and exactly one LingMind business connection is available, select it automatically.
3. If multiple connections are available, show their environment codes and ask the user to choose one.
...
- Each environment completes its own OAuth login; tokens and plans never cross connections.
